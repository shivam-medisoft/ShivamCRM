  ...
  */
		...
		function mdDateTimePicker(_ref) {
			...
			var type = _ref.type,
			    ...
			    _ref$past = _ref.past,
			    past = _ref$past === undefined ? (0, _moment2.default)().subtract(100, 'years') : _ref$past,
					...
					if (e.target && e.target.nodeName == 'SPAN' && e.target.classList.contains('mddtp-picker__cell')) {
						var picker = me._sDialog.picker,
						    ...
						    titleMonth = me._sDialog.titleMonth;
                                                    selectedDate = me._sDialog.tDate.date(day);
			...
			value: function _attachEventHandlers() {
				...
				ok.onclick = function () {
                                    ...
                                    if(selectedDate == undefined || selectedDate ==  ""){
                                        $(cntrl).val($('#currentdate').val());
                                    }else{
                                        $(cntrl).val(convertDate(selectedDate));
                                    }
                                    $(cntrl).change();
