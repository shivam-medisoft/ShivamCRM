...
function    loginmethod1(loginuser, mrno, password, name, multipleusers)
{
    ...
    var isExist = false;
    debugger;
      var loginfrom=localStorage.appname;
    var path = loginuser + "/formviewlogin?mrno=" + mrno + "&password=" + password + "&loginuser=" + loginuser + "&mobile=login&loginfrom="+loginfrom;
    debugger;
    $.ajax({
        url: getHttp(path),
        type: "GET",
...
function querrySuccess(tx, results)
{
    ...
    location.href = 'PatientLoginThirdScreen.html?myehr=1';
}
function loginmahfh(loginuser, mrno, password, name, multipleusers, logintype)
{
    ...
    var msgtextbox = "";
    var msgbutoon = "";
    if (logintype === "Mobile No" || logintype === "mobile") {
        msgvalid = "Moblie Number";
        msgtextbox = "mobile";
        msgbutoon = "btnMob";
        logintype = "Mobile No";
    } else if (logintype === "Email" || logintype === "email") {
        msgvalid = "Email"
        msgtextbox = "email"
        msgbutoon = "btnEmail";
        logintype = "Email";
    ...
    $.ajax({
        ...
        success: function (responseJson) {
            ...
            for (var m = 0; m < responseJson.length; m++) {
                ...
                localStorage.appurl = getHttp(loginuser);
                localStorage.patmrno = responseJson[m]['USID'];
                localStorage.patname = responseJson[m]['FNAME'];
                localStorage.patpassword = password;
                emrnames[0] = $("#hdnname").val();
                emrurls[0] = loginuser;
                emruhids[0] = responseJson[m]['USID'];
                emrpasswords[0] = password;
                emrlogintypes[0] = logintype;
                if (localStorage.getItem("emrnames") === null) {
                    localStorage.setItem("emrnames", JSON.stringify(emrnames));
                    localStorage.setItem("emrurls", JSON.stringify(emrurls));
                    localStorage.setItem("emruhids", JSON.stringify(emruhids));
                    localStorage.setItem("emrpasswords", JSON.stringify(emrpasswords));
                    localStorage.setItem("emrlogintype", JSON.stringify(emrlogintypes));
                } else {
                    var storedNames = JSON.parse(localStorage.getItem("emrnames"));
                    var storedUrls = JSON.parse(localStorage.getItem("emrurls"));
                    var storedUhids = JSON.parse(localStorage.getItem("emruhids"));
                    ...
                    for (i = 0; i < storedNames.length; i++) {
                        if (mrno !== storedUhids[i]) {
                            ...
                            emruhids[i] = storedUhids[i];
                            emrpasswords[i] = storedPwds[i];
                            emrlogintypes[i] = storedLogins[i];
                            isExist = false;
                        } else {
                            isExist = true;
                            existedVal = i;
                    ...
                    if (!isExist) {
                        ...
                        emrnames[emrsize] = $("#hdnname").val();
                        emrurls[emrsize] = loginuser;
                        emruhids[emrsize] = responseJson[m]['USID'];
                        emrpasswords[emrsize] = password;
                        emrlogintypes[emrsize] = logintype;
                    }
                    localStorage.setItem("emrnames", JSON.stringify(emrnames));
                    localStorage.setItem("emrurls", JSON.stringify(emrurls));
                    localStorage.setItem("emruhids", JSON.stringify(emruhids));
            ...
            if (responseJson.length === 1) {
                ...
                for (var m = 0; m < responseJson.length; m++) {
                    ...
                    multinames[m] = responseJson[m]['FNAME'];
                    multiurls[m] = loginuser;
                    multiuhids[m] = responseJson[m]['USID'];
                    multipasswords[m] = responseJson[m]['PWD'];
                    multilogintypes[m] = logintype;
                }
                localStorage.setItem("multinames", JSON.stringify(multinames));
                localStorage.setItem("multiurls", JSON.stringify(multiurls));
                localStorage.setItem("multiuhids", JSON.stringify(multiuhids));
                localStorage.setItem("multipasswords", JSON.stringify(multipasswords));
                localStorage.setItem("multilogintype", JSON.stringify(multilogintypes));
                location.href = 'addusers.html?users=' + responseJson.length + '&logintype=' + logintype;
            }
            else {
                $('.alert-warning').show('slow');
